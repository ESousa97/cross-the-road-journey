// car.js
window.Car = function(x, y, speed, carType, isFast) {
    // Defina o método ANTES de chamar!
    ...
    this.updateSpeed = function(levelMultiplier) {
        this.speed = this.baseSpeed * levelMultiplier;
        if (this.isFast) this.speed *= 1.8;
    };
    ...
    this.draw = function() {
        ...
        if (this.isFast && this.trail.length > 0) {
            for (var i = 0; i < this.trail.length; i++) {
                var alpha = map(i, 0, this.trail.length, 0, 150);
